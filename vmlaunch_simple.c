...
static int vmxon_init(void) {
    ...
    // Prepare state for VMLAUNCH/VMEXIT
    //host rip
    asm volatile("movq $0x6c16, %rdx");
    asm volatile("movq $prepare_handler, %rax");
    asm volatile("vmwrite %rax, %rdx");
    //guest rip
    asm volatile("movq $0x681e, %rdx");
    ...
    asm volatile("prepare_handler:\n");
    asm volatile("nop\n"); // to play safe
        
    // Prepare state for VMRESUME/VMEXIT
    //host rip
    asm volatile("movq $0x6c16, %rdx");
    asm volatile("movq $vmexit_handler, %rax");
    asm volatile("vmwrite %rax, %rdx");

    //guest rip
    asm volatile("movq $0x681e, %rdx");
    asm volatile("movq $guest_resume_point, %rax");
    asm volatile("vmwrite %rax, %rdx");
    ...
    for (n_round = 1; n_round < N_ITER; n_round *=2) {
        ...
        for (cur = 0; cur < n_round; cur++) {
            ...
            asm volatile("nop\n"); //will never get here
            // Guest code snippet
            asm volatile("guest_resume_point:");
